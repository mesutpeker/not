...
(function () {
    ...
    function renderStudentPage(area) {
        ...
        const quickEntry = el('section', { className: 'quick-entry-card' });
        const quickEntryHeader = el('div', { className: 'quick-entry-header' }, [
            el('div', { className: 'quick-entry-badge', textContent: 'Hızlı Giriş' }),
            el('h2', { textContent: 'Yeni öğrenciyi anında ekleyin' }),
            el('p', {
                textContent: 'Telefon ekranında ilk açılışta görünmesi için giriş alanları öne alındı. Numara ve ad-soyadı yazıp tek dokunuşla listeye ekleyebilirsiniz.'
            })
        ]);
        quickEntry.appendChild(quickEntryHeader);

        const quickEntryForm = el('div', { className: 'quick-entry-form' });
        const noField = el('label', { className: 'quick-field' }, [
            el('span', { className: 'quick-field-label', textContent: 'Öğrenci No' }),
            el('input', {
                ...
                placeholder: 'Örn. 322',
                id: 'quickAddNo',
                value: quickAddDraft.no,
                onInput: (e) => {
                    quickAddDraft.no = e.target.value;
                },
                onKeydown: (e) => {
                    if (e.key === 'Enter') {
                        e.preventDefault();
                        const nameField = $('#quickAddName');
                        if (nameField) nameField.focus();
                    }
                }
            })
        ]);
        const nameField = el('label', { className: 'quick-field quick-field-wide' }, [
            el('span', { className: 'quick-field-label', textContent: 'Ad Soyad' }),
            el('input', {
                className: 'quick-entry-input',
                type: 'text',
                ...
                onInput: (e) => {
                    quickAddDraft.name = e.target.value.toLocaleUpperCase('tr-TR');
                    e.target.value = quickAddDraft.name;
                },
                onKeydown: (e) => {
                    if (e.key === 'Enter') {
                        e.preventDefault();
                        addStudentFromQuickEntry();
                    }
                }
            })
        ]);
        const addQuickBtn = el('button', {
            className: 'btn-quick-add',
            type: 'button',
            textContent: '+ Listeye Ekle',
            onClick: () => {
                addStudentFromQuickEntry();
            }
        });
        quickEntryForm.appendChild(noField);
        quickEntryForm.appendChild(nameField);
        quickEntryForm.appendChild(addQuickBtn);
        quickEntry.appendChild(quickEntryForm);

        const quickEntryActions = el('div', { className: 'quick-entry-actions' }, [
            el('button', {
                className: 'btn-add-student btn-bulk-import',
                id: 'btnBulkImportHero',
                textContent: '📋 Toplu Liste Yapıştır',
                onClick: () => {
                    showBulkImportModal();
                }
            }),
            el('div', {
                className: 'quick-entry-note',
                textContent: `Kalan kontenjan: ${Math.max(0, 40 - appData.students.length)} öğrenci`
            })
        ]);
    ...
    function addStudentFromQuickEntry() {
        if (appData.students.length >= 40) {
            showToast('Maksimum 40 öğrenci eklenebilir.', 'error');
            return;
        }

        const no = quickAddDraft.no.trim();
        const name = quickAddDraft.name.trim();

        if (!no || !name) {
            showToast('Öğrenci no ve ad-soyad alanlarını doldurun.', 'error');
            return;
        }

        appData.students.push({ no, name });
        quickAddDraft = { no: '', name: '' };
        saveData();
        renderPage();
        showToast(`${name} listeye eklendi.`, 'success');

        setTimeout(() => {
            const nextField = $('#quickAddNo');
    ...
    function renderPerformancePage(area) {
        ...
        const mobileList = el('div', { className: 'perf-mobile-list' });
        appData.students.forEach((student, i) => {
            const card = el('div', { className: 'perf-mobile-card' });
            const header = el('div', { className: 'perf-mobile-card-header' }, [
                el('div', { className: 'perf-mobile-student' }, [
                    el('div', { className: 'perf-mobile-index', textContent: String(i + 1) }),
                    el('div', { className: 'perf-mobile-meta' }, [
                        el('div', { className: 'perf-mobile-name', textContent: student.name || '-' }),
                        el('div', { className: 'perf-mobile-no', textContent: `No: ${student.no || '-'}` })
                    ])
                ])
            ]);

            const targetArea = el('div', { className: 'perf-mobile-target' });
            targetArea.appendChild(el('div', { className: 'perf-mobile-target-label', textContent: 'Hedef Puan' }));

            const targetWrapper = el('div', { className: 'target-input-wrapper target-input-wrapper-mobile' });
            const targetInput = el('input', {
                className: 'target-input target-input-mobile',
                type: 'number',
                min: '0',
                max: '100',
                step: '1',
                placeholder: '0-100',
                id: `mobile-target-score-${i}`,
            ...
            const distributeBtn = el('button', {
                className: 'btn-distribute btn-distribute-mobile',
                title: 'Puanı yeniden dağıt',
                textContent: 'Uygula',
                onClick: () => {
                    const val = parseFloat(targetInput.value);
            ...
            targetWrapper.appendChild(targetInput);
            targetWrapper.appendChild(distributeBtn);
            targetArea.appendChild(targetWrapper);

            card.appendChild(header);
            card.appendChild(targetArea);
            mobileList.appendChild(card);
        });
        area.appendChild(mobileList);
